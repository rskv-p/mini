...
var Handlers = map[string]typ.Handler{}

func Register(name string, handler typ.Handler) {
	Handlers[strings.ToLower(name)] = handler
}

func RegisterGroup(name string, group map[string]typ.Handler) {
...
func (a *Action) handler() (typ.Handler, error) {
