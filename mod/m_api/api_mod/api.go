...
func RegisterAction(name string, handler act_type.Handler, opts APIOption) error {
	// Register in x_action
	act_core.Register(name, handler)
...
func RegisterAPI(name string, handler act_type.Handler, opts APIOption) error {
	...
	Register(name, func(args []any) (any, error) {
		...
		action := act_core.NewAction(name, ctx, args...) // Pass context as the first argument
...
type ApiModule struct {
	Module          typ.IModule
	handlerRegistry map[string]act_type.Handler
...
func (m *ApiModule) Init() error {
	...
	m.handlerRegistry = make(map[string]act_type.Handler)
...
func (m *ApiModule) GetActions() []act_type.ActionDef {
...
func registerDocsEndpoint() {
	...
	Register("api.docs", func(args []any) (any, error) {
		...
		for name := range act_core.Handlers {
