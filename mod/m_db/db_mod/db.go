...
import (
	...
	"github.com/rskv-p/mini/mod/m_db/db_core"
...
type DBModule struct {
	...
	Client       db_type.IDBClient      // Client for working with the database
	Tables       []interface{}          // List of models to handle
	ConfigClient cfg_type.IConfigClient // Client for working with configuration
...
func (m *DBModule) GetName() string {
...
func (m *DBModule) Start() error {
	...
	m.logClient.Info("Starting DB module", map[string]interface{}{"module": m.GetName()})
	return nil
}

// Stop stops the module and closes the database connection.
func (m *DBModule) Stop() error {
	m.logClient.Info("Stopping DB module", map[string]interface{}{"module": m.GetName()})
	return nil
}
...
func (m *DBModule) GetActions() []act_type.ActionDef {
...
func (m *DBModule) Init() error {
	...
	dbConfig := &db_core.DatabaseConfig{
	...
	client, err := db_core.NewDBClient(dbConfig)
	if err != nil {
...
func NewDBModule(service typ.IService, configClient cfg_type.IConfigClient, logClient log_type.ILogClient) *DBModule {
	...
	for _, action := range dbModule.GetActions() {
