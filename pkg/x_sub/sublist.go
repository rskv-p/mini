...
type Sublist struct {
	tree          *x_tree.SubjectTree[*typ.Subscription] // Tree structure for subject subscriptions
...
func NewSublist(cacheSize int) *Sublist {
	return &Sublist{
		tree:          x_tree.NewSubjectTree[*typ.Subscription](),
...
// Insert adds a typ.Subscription to the tree and updates the cache.
...
// Remove deletes a typ.Subscription from the tree.
func (s *Sublist) Remove(sub *typ.Subscription) {
	key := string(sub.Subject)
...
// Match returns typ.Subscriptions matching the given subject.
func (s *Sublist) Match(subject []byte) *SublistResult {
	key := string(subject)
	res := &SublistResult{}

	// Handle event-based subjects separately
	if strings.HasPrefix(key, "event.") {
		if subs, ok := s.wildcardCache[key]; ok {
